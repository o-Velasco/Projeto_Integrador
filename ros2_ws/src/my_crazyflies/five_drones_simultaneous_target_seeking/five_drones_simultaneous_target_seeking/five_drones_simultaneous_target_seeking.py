...
class MultiDroneTargetSeeking(Node):
    def __init__(self):
        ...
        # Per-drone state
        self.takeoff_flags = [False for _ in self.drones]
        self.flight_stages = [0 for _ in self.drones]
        self.arrived = [False for _ in self.drones]
        ...
        self.get_logger().info("Multi-drone simultaneous target seeking initialized.")
    ...
    def flight_sequence_timer(self):
        ...
        for idx, drone in enumerate(self.drones):
            if self.arrived[idx]:
                continue  # Skip if this drone already arrived

            all_arrived = False  # At least one drone still working
            ...
            if int(elapsed_time * 10) % 10 == 0 and self.has_position_data[idx]:
                ...
                self.get_logger().info(
                    f"[{drone['prefix']}] Position: {self.positions[idx][0]:.2f}, {self.positions[idx][1]:.2f}, {self.positions[idx][2]:.2f}, Stage: {self.flight_stages[idx]}"
                )

            if self.flight_stages[idx] == 0:
                # Takeoff
                if not self.takeoff_flags[idx]:
                    self.get_logger().info(f"[{drone['prefix']}] 🚁 Takeoff initiated...")
                    self.takeoff_flags[idx] = True
                ...
                if self.use_position_control and self.is_at_z(idx, self.takeoff_height):
                    ...
                    self.flight_stages[idx] = 1

            elif self.flight_stages[idx] == 1:
                if self.has_position_data[idx]:
                    ...
                    if self.use_position_control and self.is_at_target(idx):
                        ...
                        self.flight_stages[idx] = 2
                        self.arrived[idx] = True

        if all_arrived:
            self.get_logger().info("All drones have reached their targets. ✅")
            # Print final positions of all drones
            self.get_logger().info("---------------------------------------------------")
            for idx, drone in enumerate(self.drones):
                self.get_logger().info(
                    f"[{drone['prefix']}] Final Position: {self.positions[idx][0]:.2f}, {self.positions[idx][1]:.2f}, {self.positions[idx][2]:.2f}"
                )
            self.get_logger().info("---------------------------------------------------")
            self.timer.cancel()
