...
class ControlServices(Node):
    ...
    def __init__(self):
        ...
        # Store neighbor poses in a dict: {cf_id: pose}
        self.neighbor_poses = {}

        # Subscribe to neighbor odometry topics
        max_id = 5  # Adjust if you have more/less drones
        neighbor_ids = []
        if cf_id > 0:
            neighbor_ids.append(cf_id - 1)
        if cf_id < max_id:
            neighbor_ids.append(cf_id + 1)
        for n_id in neighbor_ids:
            topic = f'/crazyflie{n_id}/odom'
            self.create_subscription(
                Odometry,
                topic,
                self.make_neighbor_callback(n_id),
                10
            )

    def make_neighbor_callback(self, n_id):
        def callback(msg):
            self.neighbor_poses[n_id] = msg.pose.pose
        return callback
    ...
    def odometry_callback(self, msg):
        ...
        # Example: print relative positions to neighbors
        for n_id, pose in self.neighbor_poses.items():
            rel_x = self.current_pose.position.x - pose.position.x
            rel_y = self.current_pose.position.y - pose.position.y
            rel_z = self.current_pose.position.z - pose.position.z
