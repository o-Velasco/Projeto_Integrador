...
class ControlServices(Node):
    ...
    def __init__(self):
        ...
        # For all drones except cf0, subscribe to cf0's odometry
        self.cf0_pose = None
        if cf_id != 0:
            self.cf0_subscriber = self.create_subscription(
                Odometry,
                '/crazyflie0/odom',
                self.cf0_odometry_callback,
                10
            )

    def cf0_odometry_callback(self, msg):
        self.cf0_pose = msg.pose.pose
    ...
    def odometry_callback(self, msg):
        ...
        # If not cf0 and cf0_pose is available, compute relative position
        if hasattr(self, 'cf0_pose') and self.cf0_pose is not None:
            rel_x = self.current_pose.position.x - self.cf0_pose.position.x
            rel_y = self.current_pose.position.y - self.cf0_pose.position.y
            rel_z = self.current_pose.position.z - self.cf0_pose.position.z
